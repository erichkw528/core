...
void AStar::p_bridgeSMACConfigure()
{
  ...
  nav2_util::declare_parameter_if_not_declared(this->parent_node, name + ".downsampling_factor",
                                               rclcpp::ParameterValue(1));

  nav2_util::declare_parameter_if_not_declared(this->parent_node, name + ".angle_quantization_bins",
  ...
  nav2_util::declare_parameter_if_not_declared(this->parent_node, name + ".allow_unknown",
  ...
  nav2_util::declare_parameter_if_not_declared(this->parent_node, name + ".minimum_turning_radius",
                                               rclcpp::ParameterValue(0.2));
  this->parent_node->get_parameter(name + ".minimum_turning_radius", search_info.minimum_turning_radius);

  nav2_util::declare_parameter_if_not_declared(this->parent_node, name + ".reverse_penalty",
                                               rclcpp::ParameterValue(2.0));
  this->parent_node->get_parameter(name + ".reverse_penalty", search_info.reverse_penalty);
  nav2_util::declare_parameter_if_not_declared(this->parent_node, name + ".change_penalty",
                                               rclcpp::ParameterValue(0.5));
  this->parent_node->get_parameter(name + ".change_penalty", search_info.change_penalty);
  nav2_util::declare_parameter_if_not_declared(this->parent_node, name + ".non_straight_penalty",
  ...
  nav2_util::declare_parameter_if_not_declared(this->parent_node, name + ".analytic_expansion_ratio",
                                               rclcpp::ParameterValue(2.0));
  this->parent_node->get_parameter(name + ".analytic_expansion_ratio", search_info.analytic_expansion_ratio);

  nav2_util::declare_parameter_if_not_declared(this->parent_node, name + ".max_planning_time_ms",
...
nav_msgs::msg::Path
AStar::computeTrajectory(const std::shared_ptr<const planning_interfaces::action::TrajectoryGeneration_Goal> request)
{
  RCLCPP_DEBUG(_logger, "odom - x: [%.3f] | y: [% .3f]", request->odom.pose.pose.position.x,
               request->odom.pose.pose.position.y);
  RCLCPP_DEBUG(_logger, "next_waypoint - x: [%.3f] | y: [%.3f] | ", request->next_waypoint.pose.position.x,
  ...
  {
    p_defaultSolution(&plan_global, &request->next_waypoint, "unable to fill costmap from costmap message");
    return plan_global;
  }

  _a_star->createGraph(request->costmap.metadata.size_x, request->costmap.metadata.size_y, _angle_quantizations,
                       costmap_nav2);
  ...
  {
    RCLCPP_WARN(_logger, "start.x: [%.3f] | start.y: [%.3f]", request->odom.pose.pose.position.x,
                request->odom.pose.pose.position.y);
  ...
  double orientation_bin = tf2::getYaw(request->odom.pose.pose.orientation) / _angle_bin_size;
  while (orientation_bin < 0.0)
  {
    orientation_bin += static_cast<float>(_angle_quantizations);
  }
  unsigned int orientation_bin_id = 0;  // static_cast<unsigned int>(floor(orientation_bin));
  _a_star->setStart(mx, my, 0);         // orientation_bin_id);

  RCLCPP_DEBUG(_logger, "start - mx: [%d] | my: [%d] | orientation_bin_id: [%d]", mx, my, orientation_bin_id);
  /**
   * set goal location
   * calc distance from goal to ego so that we can find world->occupancy map
   * NOTE: we have reversed the coordinate here due to
   */
  float goal_x = request->next_waypoint.pose.position.y - request->odom.pose.pose.position.y;
  float goal_y = request->next_waypoint.pose.position.x - request->odom.pose.pose.position.x;

  RCLCPP_DEBUG(_logger, "goal - x: [%.3f] | y: [%.3f] | orientation_bin_id: [%d]", goal_x, goal_y, orientation_bin_id);

  if (!costmap_nav2->worldToMap(goal_x, goal_y, mx, my))
  {
    p_defaultSolution(&plan_global, &request->next_waypoint, "Failed to convert waypoint position to map coord");
    return plan_global;
  }

  orientation_bin = tf2::getYaw(request->next_waypoint.pose.orientation) / _angle_bin_size;
  ...
  orientation_bin_id = 0;  // static_cast<unsigned int>(floor(orientation_bin)); TODO: figure
                           // out what is orientation bin

  RCLCPP_DEBUG(_logger, "goal - mx: [%d] | my: [%d] | orientation_bin_id: [%d]", mx, my, orientation_bin_id);
  ...
  try
  {
    ...
    RCLCPP_DEBUG(_logger, "num_iteration: [%d] | tolerance: [%.3f] | resolution: [%.3f]", num_iterations, _tolerance,
                 static_cast<float>(costmap_nav2->getResolution()));
    if (!_a_star->createPath(path, num_iterations, _tolerance / static_cast<float>(costmap_nav2->getResolution())))
    {
  ...
  for (int i = path.size() - 1; i >= 0; --i)
  {
    ...
    pose.pose.position.x = path_ego.back().y() + request->odom.pose.pose.position.x;

    pose.pose.position.y = path_ego.back().x() + request->odom.pose.pose.position.y;
  ...
  {
    RCLCPP_DEBUG(_logger, "i: [%d] | x: [%.3f] | y: [%.3f]", i, plan_global.poses[i].pose.position.x,
  ...
  if (_raw_plan_publisher->get_subscription_count() > 0)
  {
    ...
    for (size_t i = 0; i < plan_global.poses.size(); i++)
    {
      ...
      ps = this->pConvertToEgoFov(plan_global.poses[i].pose, request->odom, this->tf_buffer, this->_clock);
