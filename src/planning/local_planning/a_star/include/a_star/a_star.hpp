...
public:
  AStar()
  {
    ...
    if (DEBUG)
    {
      auto ret = rcutils_logging_set_logger_level(_logger.get_name(),
  ...
  nav_msgs::msg::Path computeTrajectory(
      const std::shared_ptr<const planning_interfaces::action::TrajectoryGeneration_Goal> request) override;
...
protected:
  ...
  void fillCostmapFromMsg(nav2_costmap_2d::Costmap2D* costmap, const nav2_msgs::msg::Costmap* msg)
  {
    ...
    costmap->resizeMap(width, height, msg->metadata.resolution, msg->metadata.origin.position.x,
  ...
  Eigen::Vector2d getWorldCoords(const float& mx, const float& my, const nav2_costmap_2d::Costmap2D* costmap)
  {
    // mx, my are in continuous grid coordinates, must convert to world
    // coordinates
    double world_x = static_cast<double>(costmap->getOriginX()) + (mx + 0.5) * costmap->getResolution();
    double world_y = static_cast<double>(costmap->getOriginY()) + (my + 0.5) * costmap->getResolution();
  ...
  void removeHook(std::vector<Eigen::Vector2d>& path)
  {
    ...
    if (smac_planner::squaredDistance(path.end()[-2], path.end()[-1]) >
        smac_planner::squaredDistance(interpolated_second_to_last_point, path.end()[-1]))
    {
  ...
  void p_debugSearchInfo(smac_planner::SearchInfo searchInfo)
  {
    ...
    RCLCPP_INFO(_logger, "  Minimum Turning Radius: %.2f", searchInfo.minimum_turning_radius);
    RCLCPP_INFO(_logger, "  Non-straight Penalty: %.2f", searchInfo.non_straight_penalty);
    RCLCPP_INFO(_logger, "  Change Penalty: %.2f", searchInfo.change_penalty);
    RCLCPP_INFO(_logger, "  Reverse Penalty: %.2f", searchInfo.reverse_penalty);
    RCLCPP_INFO(_logger, "  Cost Penalty: %.2f", searchInfo.cost_penalty);
    RCLCPP_INFO(_logger, "  Analytic Expansion Ratio: %.2f", searchInfo.analytic_expansion_ratio);
  }
  void p_debugCostMapMsg(const nav2_msgs::msg::Costmap* msg)
  {
    ...
    RCLCPP_DEBUG(_logger, "  origin_x: %.3f", msg->metadata.origin.position.x);
    RCLCPP_DEBUG(_logger, "  origin_y: %.3f", msg->metadata.origin.position.y);
  }
  ...
  void p_debugCostMap(nav2_costmap_2d::Costmap2D* costmap_nav2)
  {
    ...
    RCLCPP_DEBUG(_logger, "  Resolution: %.3f", costmap_nav2->getResolution());
    RCLCPP_DEBUG(_logger, "  origin_x: %.3f", costmap_nav2->getOriginX());
    RCLCPP_DEBUG(_logger, "  origin_y: %.3f", costmap_nav2->getOriginY());
  }

  void p_defaultSolution(nav_msgs::msg::Path* plan, const geometry_msgs::msg::PoseStamped* next_waypoint,
                         const std::string reason)
  {
    RCLCPP_WARN(_logger, "Default solution emitted. Reason: [%s]", reason.c_str());

    plan->poses.push_back(*next_waypoint);  // default just emit the next
  }

  geometry_msgs::msg::PoseStamped::SharedPtr pConvertToEgoFov(geometry_msgs::msg::Pose original,
                                                              ...
                                                              rclcpp::Clock::SharedPtr clock)
  {
    ...
    {
      RCLCPP_WARN(_logger, "Could not transform [%s] to [%s]: [%s]", toFrameRel.c_str(), fromFrameRel.c_str(),
                  ex.what());
      return nullptr;
    }
